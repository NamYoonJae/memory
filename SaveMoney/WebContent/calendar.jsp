...
<html>
	...
	<body>	
		...
		<div><h3><img id ="cal_2" src="./next.png"/></h3></div>
		<div><h3><img id ="cal_1" src="./previous.png"/></h3></div>
		...
		<!-- X 버튼 -->
		<div id="cal_x">
			<img src="./letter_x.png" height="20" width="20"/>
		</div>
	...
	<script>
		...
		function allComeApp(data){
			...
			for(var i=0;i<data.ca_outcome.length;i++){
	            ...
	            if(data.ca_outcome[i].iosMemo == null){
	            	...
	            	content2 += "<td align='center'><img class='cal_memoBtn' width='50px' height='50px' src='grayBtn.png'/></td>";
	            	
        		// 만약 메모가 있을 경우 greenBtn 이미지 출력
	            }else {
	            	content2 += "<td align='center'><div onclick=addMemoPop('"+data.ca_outcome[i].iosMemo+"')>";
	            	
	            	content2 += "<img class='cal_memoBtn' width='50px' height='50px' src='greenBtn.png'/></div></td>";
			...
			for(var i=0;i<data.ca_income.length;i++){
	            ...
	            content1+="<td align='center' style='color:#2A43B4'>"+data.ca_income[i].iosM+"</td><br/>";
	         	...
	         	// 만약 메모가 없을 경우 grayBtn 이미지 출력
	            if(data.ca_income[i] == null){
	            	// div 클릭 시 memo Popup 출력
	            	console.log("메모가 없을 경우");
	            	content1 += "<td align='center'><img class='cal_memoBtn' width='50px' height='50px' src='grayBtn.png'/></td>";
	            	
        		// 만약 메모가 있을 경우 greenBtn 이미지 출력
	            }else {
	            	content1 += "<td align='center'><div onclick=addMemoPop('"+data.ca_income[i].iosMemo+"')>";
	            	content1 += "<img class='cal_memoBtn' width='50px' height='50px' src='greenBtn.png'/></div></td>";
	            	console.log("메모가 있을 경우");
	            }
			...
			console.log("수입 append 시작");
			 ...
			 for(var i=0;i<data.ca_income.length;i++){
	            ...
	            if(data.ca_income[i].iosMemo == null){
	            	// div 클릭 시 memo Popup 출력
	            	console.log("메모가 없을 경우");
	            	content += "<td align='center'><img class='cal_memoBtn' width='50px' height='50px' src='grayBtn.png'/></td>";
	            	
         		// 만약 메모가 있을 경우 greenBtn 이미지 출력
	            }else {
	            	content += "<td align='center'><div onclick=addMemoPop('"+data.ca_income[i].iosMemo+"')>";
	            	
	            	content += "<img class='cal_memoBtn' width='50px' height='50px' src='greenBtn.png'/></div></td>";
	            	console.log("메모가 있을 경우");
	            }
			...
			console.log("지출 append 시작");
			 ...
			 for(var i=0;i<data.ca_outcome.length;i++){
	            ...
	            if(data.ca_outcome[i].iosMemo == null){
	            	...
	            	content += "<img class='cal_memoBtn' width='50px' height='50px' src='grayBtn.png'/></td>";
		...
		function addMemoPop(incomedata){
			...
			$(".cal_x2").html("<img src ='./letter-x2.png' height='20' width='20'/>");
			$("td").unbind("hover");
		...
		function spendMemoPop(outcomedata){
			...
			$("#cal_back2").css("display", "block");
			$(".cal_x2").css("display", "block");
			$(".cal_memoPop").html("<p>당신의 메모</p><hr/><br/>"+outcomedata+"<div class='cal_x2'><img class='cal_x2img' src ='./letter-x2.png' height='20' width='20'/></div>");
