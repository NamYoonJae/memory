...
public class LongAddDAO {
	...
	/*public ArrayList<longTurmDTO> popchoice(String memberIdx) {
		
		ArrayList<longTurmDTO> poplongTurm = new ArrayList<longTurmDTO>();
		String sql = "SELECT *FROM longTerm_tb WHERE longIdx=?";
		
		try {
			ps = conn.prepareStatement(sql);
			ps.setString(1,memberIdx);
			rs=ps.executeQuery();
			while(rs.next()) {
				System.out.println(rs.getInt("longIdx"));
				...
				dto.setLongTerm(rs.getInt("longTerm"));					
				poplongTurm.add(dto);
				System.out.println(poplongTurm.get(0).getLongSubject());
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		...
		System.out.println(poplongTurm);
		return poplongTurm;
