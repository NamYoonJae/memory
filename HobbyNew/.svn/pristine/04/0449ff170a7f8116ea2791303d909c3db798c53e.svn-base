...
<html>
	...
	<jsp:include page="../loginBox.jsp"></jsp:include>
		...
		<div id="tables">
			<form id="freeWriteForm" name="freeWriteForm" action=b_freeWrite method="POST" enctype="multipart/form-data">
	...
	function freeWrite() {
		...
		var sort = document.freeWriteForm;
		
		if(sort.cn_index.value ==0){
			alert("분류를 선택해주세요")
		}else if($("#fb_title").val() == ""){
			alert("말머리를 입력해주세요")
		}else if($("#cb_subject").val() == ""){
			alert("제목을 입력해주세요")
		}else if($("#cb_content").val() == ""){
			alert("내용을 입력해주세요")
		}else{
			console.log("submit");
